        ...
        <div className='relative w-fit z-10'>
          ...
          >
            <Listbox.Options className={`custom-filter__options`}>
              {options.map(option => (
                <Listbox.Option className={({ active }) => `relative cursor-default select-none py-2 px-4 ${active ? 'bg-primary-blue text-white' : 'text-gray-900'}`} value={option} key={option.title}>
                  {({ selected }) => (
